...
def convert_url_to_filename(url):
    ...
    filename = filename.replace(
        " ", "-"
    )  # to replace spaces in file names with dashes.
    filename = filename.replace(
        "/", "-"
    )  # to replace slashes in file names with dashes.
...
def get_given_queries(
    query_file: str, user, topic_id, task_a2: bool = True
) -> list[tuple[str, int]]:
    given_queries: list = []
    ...
    for line in queries_file:
        ...
        line_topic = line[2]
        line_terms = line[3]
        if line_user == user and line_topic == topic_id:
